    ...
    try {
      qrRef.current.toDataURL(async (data: string) => {
        ...
        if (isAvailable) {
          ...
          // Note: Sharing base64 directly might not work on all platforms
          // Consider using FileSystem to save temporarily first
        } else {
  ...
  const saveToGallery = async () => {
    ...
    try {
      qrRef.current.toDataURL(async (data: string) => {
        ...
        // In production, you would save the base64 data as an image file
      });
    } catch {
